...
public class BankCardUtils {
    ...
    public static boolean checkBankCard(String cardNum) {
        ...
        return bit != 'N' && cardNum.charAt(cardNum.length() - 1) == bit;
    }
     ...
     * @return 银行名称
     */
    public static String getNameOfBank(String cardNum) {
        int index = -1;

        if (cardNum == null || cardNum.length() < 16 || cardNum.length() > 19) {
            return null;
        }

        //6位Bin号
        String cardBin_6 = cardNum.substring(0, 6);
        ...
        //8位Bin号
        String cardBin_8 = cardNum.substring(0, 8);
        for (int i = 0; i < bankBin.length; i++) {
     ...
     * @param cardNum 未校验卡号
     */
    private static char getBankCardCheckCode(String cardNum) {
        if (cardNum == null || cardNum.trim().length() == 0
                || !cardNum.matches("\\d+")) {
            //如果传的不是数据返回N
            return 'N';
        }
        char[] chs = cardNum.trim().toCharArray();
        int luhmSum = 0;
